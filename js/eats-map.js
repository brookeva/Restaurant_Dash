function initializeeatsMap(eventBus) {
  ...
  //   const [lat, lon] = geojson.geometry.coordinates;
  ...
  eatsMap.EatsLayers = {};
   ...
   // showEatsOnMap(Restaurants, eatsMap);
...
function redrawEatsOnMap(Restaurants, map) {
  ...
  // for (const Place of Restaurants) {
  //   const id = Place.properties['Name'];
  //   const layer = map.EatsLayers[id];
  //   if (layer) {
  //     layer.resetStyle();
  //   }
...
function showEatsOnMap(Restaurants, map) {
  ...
  // const ids = new Set(Restaurants.map((s) => s['Name']));

  // // Remove features with IDs not in the schools array
  // for (const id of Object.keys(map.EatsLayers)) {
  //   if (!ids.has(id)) {
  //     const layer = map.EatsLayers[id];
  //     map.removeLayer(layer);
  //     delete map.EatsLayers[id];
  //   }
  // }

  // // Add new features for IDs not in the layers
  // for (const place of Restaurants) {
  //   const feature = makeEatsFeature(place);
  //   const id = feature.id;
  //   if (map.EatsLayers[id] === undefined) {
  //     map.EatsLayers[id] = L.geoJSON(feature, {
  //       pointToLayer: (p, ll) => L.circleMarker(ll),
  //       style: (f) => getEatsStyle(f, map),
  //     })
  //       .on('click', () => {
  //         const detail = isHighlightedOnMap(place, map)
  //           ? { 'added': [], 'removed': [place] }
  //           : { 'added': [place], 'removed': [] };
